...
export const PhoneSimulator = ({ className = "" }) => {
  ...
  return (
    <div className={`relative w-80 max-w-xs bg-black rounded-[2.5rem] p-2 shadow-2xl border-4 border-gray-300 ${className}`}>
      {/* Notch superior */}
      <div className="w-24 h-2 rounded-b-xl bg-black mx-auto mb-1"></div>

             {/* Header celular  */}
          <div className="bg-white rounded-[2rem] overflow-hidden">
            <div className="bg-primary py-2 text-center text-white font-medium text-[10px] rounded-t-[2rem]">
        ...
        <div className="flex">
          {/* Barra lateral */}
          <div className="w-16 bg-gray-100 font-medium p-2 text-[8px] space-y-3 text-gray-400">
            ...
            <div className="flex relative">
              <span className="relative font-bold text-gray-600">
                🔔 Alerta
              </span>
            </div>
          </div>
          ...
          <div className="flex-1 p-2 grid grid-cols-1 gap-2 text-xs">
            ...
            <div className="bg-white rounded-lg border border-gray-200 shadow-lg hover:shadow-xl transition-all duration-300 p-2 flex justify-between items-center">
              <span className="text-[10px] text-gray-500">🌡️ Temperatura</span>
              <span className="text-[10px] font-medium text-red-500">40.0°C</span>
            </div>

            {/* Card Humedad */}
            <div className="bg-white rounded-lg border border-gray-200 shadow-lg hover:shadow-xl transition-all duration-300 p-2 flex justify-between items-center">
              <span className="text-[10px] text-gray-500">💧 Humedad</span>
              <span className="text-[10px] font-medium text-blue-500">20%</span>
            </div>

            {/* Card EC */}
            <div className="bg-white rounded-lg border border-gray-200 shadow-lg hover:shadow-xl transition-all duration-300 p-2 flex justify-between items-center">
              <span className="text-[10px] text-gray-500">⚡ EC</span>
              <span className="text-[10px] font-medium text-green-500">0.00PPM</span>
            </div>

            {/* Gráficos */}
            <div className="bg-gray-50 rounded p-1">
              <p className="text-[10px] font-medium text-gray-600 mb-1">Gráfica de Temperatura</p>
              <div className="w-full">
                <ResponsiveContainer width="100%" height={80}>
                  <LineChart data={data} margin={{ left: 0, right: 0 }}>
                    ...
                    <YAxis fontSize={7} />
                    <Tooltip />
                    <Line type="monotone" dataKey="temperatura" stroke="#f87171" strokeWidth={2} dot={false} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </div>
            ...
            <div className="bg-gray-50 rounded p-1">
              <p className="text-[10px] font-medium text-gray-600 mb-1">Gráfica de Humedad</p>
              <div className="w-full">
                <ResponsiveContainer width="100%" height={80}>
                  <LineChart data={data} margin={{ left: 0, right: 0 }}>
                    <CartesianGrid strokeDasharray="3 3" />
                    ...
                    <YAxis fontSize={7} />
                    <Tooltip />
                    <Line type="monotone" dataKey="humedad" stroke="#60a5fa" strokeWidth={2} dot={false} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </div>
