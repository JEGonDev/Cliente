...
export const GroupDetailsView = () => {
  ...
  // Hook de grupo: ahora fetchGroups viene del contexto y refresca la info global
  const {
    selectedGroup: group,
    groupLoading,
    groupError,
    ...
    cancelEdit,
    clearMessages,
    fetchGroups, // <-- Nuevo: refresca la info del grupo tras editar/eliminar
  } = useGroup(groupId);
  ...
  if (result) {
    await fetchGroups(); // <--- refresca la data en el contexto
    setShowEditModal(false); // Cierra el modal
    // No navegues a otro lado, la misma vista mostrará los cambios al tomar el grupo actualizado del contexto
  }
};

  // const handleEditSubmit = async () => {
  //   // Ahora handleUpdateGroup recibe el groupId y los datos
  //   const result = await handleUpdateGroup(group.id, formData);
  //   if (result) {
  //     // Refresca la info global tras editar
  //     fetchGroups();
  //     setTimeout(() => {
  ...
  const handleConfirmDelete = async () => {
    ...
    if (success) {
      // Refresca los grupos tras eliminar
      fetchGroups();
      setShowDeleteConfirmation(false);
      // El hook se encargará de la redirección si corresponde
    }
  };
  ...
  return (
    <div className="max-w-5xl mx-auto my-8 p-4 bg-white rounded shadow">
      ...
      <div className="mt-10">
        ...
        <ThreadList groupId={stableGroupId} refetchRef={reloadThreadsRef} />
      </div>
...
//             setShowThreadModal(false);
//             if (reloadThreadsRef.current) {
//               reloadThreadsRef.current(); // <-- recarga la lista de hilos
//             }
//           }}
//         />
//       )}
